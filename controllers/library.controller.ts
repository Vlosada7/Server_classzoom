...
const addLessonId = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const filterLessons = library.lessons.filter((lesson:any) => lesson.id === lessonId);
    if (filterLessons.length) {
      throw new Error('lesson already exists');
    }
    const lessonIds = library.lessons.map((el:any) => ({ id: el.id }));
    ...
    const hasLesson = library2.lessons.some((noteBook:any) => noteBook.id === lessonId);
    if (!hasLesson) {
